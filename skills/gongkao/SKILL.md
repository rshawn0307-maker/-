...
为公考面试结构化培训提供全产品支持，覆盖从单题生成到完整讲义的6个工作流：
...
**调度规则**：用户意图匹配多个工作流时，按 A→B→C→D→E→F 顺序询问用户优先执行哪个。
...
- 破折号（human-writing 1.1 起清零）→ 全部改为逗号或句号断句 → 重跑确认
...
3. **破折号**（"——这是关键"）→ human-writing 1.1 起全部清零，改为逗号或句号
...
**改写后自检**：总字数仍在 800-1000 之间；读起来像人话（用 human-writing 的 scripts/check_prose.py 复核，同仓库安装时位于 `../human-writing/scripts/check_prose.py`；human-writing 1.1.0 起冒号/破折号/翻案腔为硬禁令，全部清零）
