...
为公考面试结构化培训提供 **全产品支持**，覆盖从单题生成到完整讲义的 6 个工作流。包括题目生成、方法论提炼、框架模板、过渡句库、素材库生成、讲义组装/导出。覆盖综合分析、组织计划、人际关系、应急应变、自我认知、言语表达六类题型，每道题包含 frontmatter 元信息、题干（≤80字）、思路大纲、800-1000字逐字稿，集成 Python 多模式验证脚本进行 AI 痕迹扫描和用语禁忌检查。

---

## 功能亮点

- **6 个工作流全覆盖**：题目生成 / 方法论 / 框架模板 / 过渡句 / 素材库 / 讲义组装
...
- **模块化架构**：工作流 B-F 的详细模板放在 `references/` 目录，SKILL.md 只保留调度表和流程步骤
...
├── scripts/
│   └── validate_question.py          # Python 验证脚本（5种模式）
└── test-prompts.json                 # 测试用例（8 个 prompt）
```
...
- **当前版本**：v2.0.0
- **更新日期**：2026-08-02
- **变更摘要**：从单一题目生成器升级为全产品工具，新增 5 个工作流（方法论/框架/过渡句/素材/讲义组装），验证脚本支持 5 种模式，统一字数标准为 800-1000
